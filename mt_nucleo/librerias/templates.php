	...
	*/
	class templates{
		...
		public function sustDate($cadena){
			...
			$coincidenciasSimple = $this->getCadenaPatron(
			...
			if( $coincidenciasSimple ){
				foreach ($coincidenciasSimple as $k => $v) {
		...
		public function truncate($cadena){
			$remplazos = array();
			$coincidencias = $this->getCadenaPatron(
				$cadena, '/\{(.*?)\|TRUNCATE\|([0-9]+)\}/is', true
			);
			if( $coincidencias ){
				foreach ($coincidencias as $k => $v) {
					if( strlen($v[1]) > $v[2] )
						$remplazos[$v[0]] = mb_strimwidth($v[1], 0, $v[2]).'...';
					else $remplazos[$v[0]] = $v[1];
				}
			}
			if( $remplazos ){
				$render = str_replace(
		...
		public function escape($cadena){
			$remplazos = array();
			$coincidencias = $this->getCadenaPatron(
				$cadena, '/\{(.*?)\|ESCAPE\}/is', true
			);
			if( $coincidencias ){
				foreach ($coincidencias as $k => $v) {
					$remplazos[$v[0]] = htmlentities($v[1]);
				}
			}
			if( $remplazos ){
				$render = str_replace(
